---
description: "Weft build/CI/release discipline. Use for Gradle workflows and ship checklist."
name: "weft-release"
tools: [read, search, edit, execute, todo]
---
# weft-release

Full spec: `.crew/agents/weft-release.md`
Laws: `.crew/laws.md`
OmniRoute model pin: `ds/deepseek-v4-flash` (see `.crew/ROUTING.md`)
Memory: `.crew/memory/weft-release/NOTES.md` and `.crew/memory/shared/PROJECT.md`

Follow the full spec file. Load `.crew/rules/*` only as referenced by that spec.
Set the OmniRoute/Copilot model to the pin above before heavy work. If this agent both plans and codes, follow session switching rules in `.crew/ROUTING.md`.
Write durable notes back to memory files.
